...
class GCP_Metrics:
    def __init__(self, vm_external_ip, kiali_port, prometheus_port, namespace):
        ...
        self.total_size_messages = 0.0
        self.total_bytes_exchanged = 0.0
        ...
        # Affinities for iXen
        self.iXenAffinities_RPS = {}
        self.iXenAffinities_Normalized = {}
        self.iXen_RPS_Collection = {}
        self.iXen_Normalized_Collection = {}
    ...
    def kube_service_affinities(self):
        ...
        query_string_kiali = {"duration": "1h", "namespaces": self.namespace,
    ...
    def kube_total_affinity_bytes(self):
        ...
        for x in range(total_queries):
            ...
            self.total_bytes_exchanged += float(self.traffic_requested_bytes[source_app][dest_app]) + float(self.traffic_responsed_bytes[source_app][dest_app])
            self.total_size_messages += float(self.traffic_requested_count[source_app][dest_app]) + float(self.traffic_responsed_count[source_app][dest_app])
        ...
        # Calculate affinities according to Sampaio -> normalized affinities bytes A->B + normalized affinities count A->B
        bytes_metric = 0.0
        count_metric = 0.0
        for source_app in self.traffic_requested_bytes:
            for dest_app in self.traffic_requested_bytes[source_app]:
                bytes_metric = (float(self.traffic_requested_bytes[source_app][dest_app]) + float(self.traffic_responsed_bytes[source_app][dest_app]))/self.total_bytes_exchanged
                count_metric = (float(self.traffic_requested_count[source_app][dest_app]) + float(self.traffic_responsed_count[source_app][dest_app]))/self.total_size_messages
                self.total_affinities_bytes[source_app][dest_app] = format(float(bytes_metric * 0.5 + count_metric * 0.5), '.3f')
        ...
        # Only for iXen
        # self.iXenAffinities_RPS = self.service_affinities
        # self.iXenAffinities_RPS['cygnusproxy'] = {}
        # self.iXenAffinities_RPS['cygnusproxy']['cygnus'] = self.iXenAffinities_RPS['orion']['cygnus']
        # self.iXenAffinities_RPS['orion']['cygnusproxy'] = self.iXenAffinities_RPS['orion'].pop('cygnus')
        # self.iXenAffinities_RPS['queryingsensorsproxy'] = {}
        # self.iXenAffinities_RPS['queryingsensorsproxy']['queryingsensors'] = self.iXenAffinities_RPS['apache'][
        #     'queryingsensors']
        # self.iXenAffinities_RPS['apache']['queryingsensorsproxy'] = self.iXenAffinities_RPS['apache'].pop(
        #     'queryingsensors')
        # self.iXenAffinities_RPS['keyrock']['mysql'] = self.iXenAffinities_RPS['apache']['keyrock']
        # self.iXenAffinities_RPS['orion']['mongo'] = self.iXenAffinities_RPS['orionproxy']['orion']
        # self.iXenAffinities_RPS['apache'].pop('apache')
        # pprint.pprint(self.iXenAffinities_RPS)
        #
        # self.iXenAffinities_Normalized = self.total_affinities_bytes
        # self.iXenAffinities_Normalized['cygnusproxy'] = {}
        # self.iXenAffinities_Normalized['cygnusproxy']['cygnus'] = self.iXenAffinities_Normalized['orion']['cygnus']
        # self.iXenAffinities_Normalized['orion']['cygnusproxy'] = self.iXenAffinities_Normalized['orion'].pop('cygnus')
        # self.iXenAffinities_Normalized['queryingsensorsproxy'] = {}
        # self.iXenAffinities_Normalized['queryingsensorsproxy']['queryingsensors'] = self.iXenAffinities_Normalized['apache'][
        #     'queryingsensors']
        # self.iXenAffinities_Normalized['apache']['queryingsensorsproxy'] = self.iXenAffinities_Normalized['apache'].pop(
        #     'queryingsensors')
        # self.iXenAffinities_Normalized['keyrock']['mysql'] = self.iXenAffinities_Normalized['apache']['keyrock']
        # self.iXenAffinities_Normalized['orion']['mongo'] = self.iXenAffinities_Normalized['orionproxy']['orion']
        # self.iXenAffinities_Normalized['apache']['mongo'] = self.iXenAffinities_Normalized['apache']['queryingsensorsproxy']
        # self.iXenAffinities_Normalized['apache'].pop('apache')
        # pprint.pprint(self.iXenAffinities_Normalized)
        # Assemble all affinities in one matrix in decent order
        # for source_key in self.iXenAffinities_Normalized:
        #     for destination_key in self.iXenAffinities_Normalized[source_key]:
        #         self.iXen_RPS_Collection[source_key + "->" + destination_key] = float(
        #             self.iXenAffinities_RPS[source_key][destination_key])
        #         self.iXen_RPS_Collection = dict(
        #             sorted(self.iXen_RPS_Collection.items(), key=operator.itemgetter(1), reverse=True))
        #         self.iXen_Normalized_Collection[source_key + "->" + destination_key] = float(
        #             self.iXenAffinities_Normalized[source_key][destination_key])
        #         self.iXen_Normalized_Collection = dict(
        #             sorted(self.iXen_Normalized_Collection.items(), key=operator.itemgetter(1), reverse=True))
