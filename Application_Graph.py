...
class Application_Graph():
    ...
    def find_graph_nodes(self):
        for x in range(len(self.service_list)):
            ...
            #print(self.service_to_id[self.service_list[x]])
            #print(self.id_to_service[x])

    # Insert Edges
    def find_graph_edjes(self):
        for source in self.service_affinities:
            for dest in self.service_affinities[source]:
                weight=float(self.service_affinities[source][dest])
                print(weight)
                print(source)
                print(dest)
                print(self.service_to_id)
                print(self.service_to_id)
                self.G.add_edge(self.service_to_id[source], self.service_to_id[dest],weight=weight)
                #self.G[self.service_to_id[source]][self.service_to_id[dest]]['weight'] = float(self.service_affinities[source][dest])
            print(source)
